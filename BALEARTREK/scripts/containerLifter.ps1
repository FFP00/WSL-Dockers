...
$userWSL    = wsl -d Debian bash -lc 'whoami'
...
$rutaConfigDockerfile   = "$PSScriptRoot\..\templates\Dockerfile"
$rutaDockerfile         = "$PSScriptRoot\..\linux\Dockerfile-PHP-BUN"
...
# --- Generar docker-compose.yml desde plantilla ---
(Get-Content $rutaConfigDockerfile -Raw) `
    -replace '\$\{userWSL\}', $userWSL `
    -replace '\$\{UID\}', $uID |
    Set-Content $rutaDockerfile    
